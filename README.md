...
For a complete backend-to-SDK workflow, try the [NestJS investment-platform example](examples/nest-market/README.md): a real backend generates OpenAPI, Accord generates its SDK, and a React workspace uses the generated hooks to browse offerings, manage subscriptions and upload documents. After building, `pnpm example:nest:dev` runs both servers with seeded data and randomized latency.
...
Parameter metadata omits default serialization settings: `{ name: "id" }` is enough for ordinary values. Content-encoded parameters use string codecs such as `{ name: "filter", codec: "json" }` or `{ name: "count", codec: "text" }`. Encodings needing settings, such as XML, retain a codec object. This applies to path, query, header and cookie parameters. Numeric/boolean text input types still come from the semantic model; request metadata needs no response-decoding hints.

Path and query inputs retain their schema types: `integer`/`number` become `number`, booleans stay `boolean`, arrays retain their item types, and objects retain their properties. Enums and unions are preserved; dates and UUIDs remain strings. For example, `{ id: 42, limit: 0, active: false }` can produce `/users/42?limit=0&active=false`. The metadata needs no numeric/boolean codec for ordinary parameters. TypeScript does not enforce integerness or numeric bounds, and `number` has JavaScript's precision limits.

Nullable query schemas are supported in both OpenAPI 3.0 and 3.1. For ordinary style encoding, Accord emits `null` as an empty value (`name=`) and omits absent/`undefined` values; JSON content encoding preserves literal `null`. For nested objects or arrays inside objects, use an explicit content encoding such as `application/json`: OpenAPI's [`deepObject` style](https://spec.openapis.org/oas/v3.1.1.html#style-values) defines only scalar object properties.
