...
import { interpolatePath, type PathParameter } from "../src/index.js"
...
describe("path serialization properties", () => {
  it("leaves no unresolved placeholders after valid input", () => {
    fc.assert(
      fc.property(
        fc
          .array(fc.stringMatching(/^[A-Za-z][A-Za-z0-9_]{0,12}$/), { minLength: 1, maxLength: 8 })
        ...
        (names, values) => {
          ...
          const descriptors: PathParameter[] = names.map((name) => ({
            name,
          }))
