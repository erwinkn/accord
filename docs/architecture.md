...
`OperationModel` ties parameters, request/response schemas, representations, public names, and the runtime plan together. A representation carries a codec decision such as JSON, text, bytes, a form, or XML. Parameter and form styles live in those transport decisions. The client never ships the source graph.
...
The output combines ordinary exported TypeScript types with `defineEndpoint<Contract, "query" | "mutation">({ plan, validators? })`. The contract has the argument tuple, input, successful result, error payload, per-status payloads, and full-response union. Its type-only marker disappears from JavaScript.
