...
Endpoint definitions use `defineEndpoint<Contract, "query" | "mutation">({ method, path, ... })`. The contract has the argument tuple, input, successful result, error payload, per-status payloads, and full-response union. It exists only in TypeScript: the object has no `contract` or `plan` wrapper; `kind` is `query` or `mutation`, and `id` is the operation ID. `defineEndpoint` adds an internal symbol so the client can distinguish an endpoint from a namespace. Validation lives in optional response `schema` entries. `defineApi(scope, { ...namespaces })` wraps the exported API once in its entry module. The scope is a fingerprint of the API contract (or the configured `apiId`) used in React Query cache keys. The wrapper copies namespace and endpoint objects and attaches the scope through an internal endpoint symbol, so endpoints taken from `api` retain their identity when used independently. Imported endpoint declarations are not mutated, allowing the same definitions to be assembled into separate APIs. Runtime consumers should take endpoints from the exported `api`; endpoint modules are unscoped building blocks. A batch allocator gives schemas short names, qualifying every participant in a conflict.
