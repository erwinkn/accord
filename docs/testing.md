...
Ordinary consumers use `skipLibCheck` **only for dependency `.d.ts` revalidation**, not generated source or consumer source. A separate `types.published-declarations` contract runs with it disabled. This is intentional: a bad emitted declaration should be visible once as its own contract violation rather than conceal every other wire invariant. The declaration audit passes with TanStack Query 5.102.8; the obsolete missing data-tag symbol baseline has been removed. Do not delete this audit or claim ordinary consumer checks validate declaration internals.
