...
| `pnpm test:nest` | Real NestJS server, exported OpenAPI regeneration, SDK/validator reproducibility, 20 operations, Multer binary uploads, React Query calls, seed isolation and randomized latency |
| `pnpm test:nest:web` | React app against a real Nest server on desktop/mobile Chromium: filters, validation errors, subscriptions, uploads/downloads, reload persistence and API failure recovery; install Playwright Chromium first |
